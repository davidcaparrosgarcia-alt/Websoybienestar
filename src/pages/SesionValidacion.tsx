...
export default function SesionValidacion() {
  ...
  const initialMode = modeParam === "reservation" ? "reserva" : (modeParam === "full" ? "unico" : (modeParam === "saldo" || modeParam === "balance" ? "saldo" : "unico"));
  ...
  const [paymentMode, setPaymentMode] = useState<"unico" | "reserva" | "saldo">(
    initialMode === "saldo"
      ? "saldo"
      : initialMode === "reserva"
      ? "reserva"
          ...
          {isReturningFromSuccessfulCardPayment ? (
            <div className="bg-surface-container-lowest p-8 md:p-12 rounded-[2rem] shadow-xl border border-outline-variant/10 text-center flex flex-col items-center">
              ...
              <p className="text-xl text-on-surface-variant font-light leading-relaxed max-w-lg mb-8">
                Has completado el pago de <strong>{plan.name}</strong> — {paymentMode === "unico" ? "pago único" : (paymentMode === "reserva" ? "reserva" : "saldo restante")}.
              </p>
            ...
            <div className="mt-8 bg-surface-container-lowest p-8 rounded-2xl flex flex-col md:flex-row justify-between items-center gap-8 shadow-lg border border-outline-variant/10">
              <div>
                ...
                <div className="flex items-baseline gap-2">
                  ...
                  <span className="text-sm text-on-surface-variant bg-surface-container px-2 py-1 rounded-md">{paymentMode === "unico" ? "pago único" : (paymentMode === "reserva" ? "de reserva" : "saldo restante")}</span>
                </div>
                <p className="text-xs text-on-surface-variant/80 font-light mt-1.5">
