                ...
                {ALIMENTACION_EMOCIONAL_FAQS.map((item, index) => (
                  <div key={item.question}>
                    <button
                      ...
                      aria-controls={`faq-alimentacion-emocional-answer-${index}`}
                    ...
                    <div
                      id={`faq-alimentacion-emocional-answer-${index}`}
                      aria-hidden={openFaqIndex !== index}
                      className={`overflow-hidden transition-[max-height] duration-300 ease-in-out ${
                        openFaqIndex === index ? "max-h-[5000px] pb-8" : "max-h-0 pb-0"
                      } px-8 md:px-10 text-on-surface-variant font-body leading-relaxed whitespace-pre-line text-base md:text-lg`}
                    >
                      {item.answer}
                    </div>
                  </div>
